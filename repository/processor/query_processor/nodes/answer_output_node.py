...
from repository.utils.mongo_history_util import save_chat_message
from repository.utils.task_util import set_task_result
from repository.utils.sse_util import push_sse_event, SSEEvent
from repository.prompts.query_prompt import ANSWER_PROMPT
...
class AnswerOutPutNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 3. 判断state中是否有answer
        if state.get('answer'):
            # 将答案推送出去，只处理非流式调用，如果是流式调用，之后关闭SSE通道的时候顺便推送
    ...
    def _push_exist_answer(self, task_id: str, is_stream: bool, state: QueryGraphState):
        ...
        # 判断是非流式【普通的任务队列：任务结果队列_tasks_result】
        if not is_stream:
