...
import logging
from repository.processor.query_processor.config import QueryConfig, get_config
from repository.processor.query_processor.exceptions import QueryProcessError
from repository.utils.task_util import add_running_task, add_done_task, get_task_status, get_running_task_list, \
    get_done_task_list
from repository.utils.sse_util import push_sse_event, SSEEvent
...
class BaseNode(ABC):
    ...
    def __call__(self, state: T) -> T:
        ...
        try:
            ...
            if task_id:
                add_done_task(task_id, self.name)  # 当前已完成的节点加入
                # 如果是流式
                if is_stream:
                    self._push_progress(task_id)
