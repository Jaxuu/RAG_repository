...
from repository.processor.import_processor.exceptions import StateFieldError, FileProcessingError
...
class _MdFileHandler:
    ...
    def validate_and_read_md(self, state: ImportGraphState) -> Tuple[str, Path, Path]:
        ...
        try:
            ...
            self.logger.error(f"读取md文件：{md_path_obj.name}失败：{e}")
...
class _ImageScanner:
    ...
    2. 去到md文件中定位图片的位置
    3. 获取该图片在md中的上下文内容（给VLM模型提供上下文，调高识别结果准确度）
    4. 最终组装所有图片的上下文内容（List）
    """
    def __init__(self, logger: Logger):
        self.logger = logger

    def scan_image_dir(self, image_dir_obj: Path, md_content: str, image_extensions: Set[str], img_content_length: int) -> List[ImageInfo]:
        """
        主要职责：
        1. 根据图片目录，得到该目录下有效的图片文件
        2. 去到md文件中定位图片的位置
        ...
        Args:
            image_dir_obj: 图片目录对象
            md_content: md内容
            image_extensions: 允许的图片后缀格式集合
            img_content_length: 图片上下文长度
        ...
        for img_path in image_dir_obj.iterdir():
            ...
            if not img_path.is_file():
                self.logger.info(f"过滤图片：{img_path.name}，因为不是文件")
                continue
            # 过滤不合法的文件
            if not img_path.suffix in image_extensions:
                self.logger.info(f"过滤图片：{img_path.name}，因为后缀不是允许的")
                continue

            # 找该图片上下文
            ctx = self._find_context(img_path.name, md_content, img_content_length)
            if not ctx:
                self.logger.info(f"过滤图片：{img_path.name}，因为该图片未被引用")
                continue

            # 封装ImageInfo对象
            img_info_list.append(ImageInfo(name=img_path.name, path=str(img_path), imag_context=ctx))

        self.logger.info(f"成功扫描图片目录，共扫描到{len(img_info_list)}张图片")
    ...
    def _find_context(self, img_name:str, md_content:str, img_content_length:int)-> Optional[ImageContext]:
        ...
        for md_idx,md_line in enumerate(md_lines):
            ...
            # 下文标题作为结束索引
            post_index = self._find_heading_down(md_lines, md_idx)
            post_lines = md_lines[md_idx+1:post_index]
    ...
    def _extract_limited_context(self, extracted_md_lines:List[str], img_content_length:int, direction:str)->str:
        ...
        for context in contexts:
            if total + len(context) > img_content_length and selected:
                break
...
class _VLMSummarizer:
    ...
    def _summary_all(self, document_name:str, img_info_list:List[ImageInfo], vl_model:str)->Dict[str,str]:
        ...
        self.logger.info(f"生成{len(summaries)}张图片的摘要")
        return summaries
...
class _MarkDownToImgNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 1.操作md_handler
        self.log_step("step1", message="读取md内容、路径以及图片的目录")
        ...
        # 2. 操作image_scanner
        self.log_step("step2", message="开始准备扫描图片目录")
        image_info_list:List[ImageInfo] = self._image_scanner.scan_image_dir(image_dir_obj,
                                                                             md_content,
        ...
        new_md_content = self._image_uploader.upload_and_replace(md_path_obj.stem, md_content, image_info_list, summaries, config.get_minio_base_url(), config.minio_bucket)
