...
from repository.core.paths import get_local_base_dir
from repository.processor.import_processor.exceptions import FileProcessingError
from repository.utils.client.storage_clients import StorageClients
from repository.processor.import_processor.main_graph import import_app
from repository.utils.task_util import update_task_status, add_running_task, add_done_task, add_node_duration, \
    ...
    def save_upload_file_to_local(self, file: UploadFile, file_dir: str) -> str:
        ...
        try:
            with  open(import_file_path, "wb") as f:
                # shutil.copyfileobj() 不同的操作系统以及不同python版本都可以分批次的写入（windows版本以及3.7以上的sdk版本:1m）,防止oom
