...
from repository.core.paths import get_front_page_dir
from repository.schema.upload_schema import UploadResponse,TaskStatusResponse
from repository.service.upload_service import UpLoadService
from repository.core.deps import get_upload_file_service
from repository.utils.task_util import get_task_info
...
def create_app():
    ...
    # 1. 实例化
    app = FastAPI(description="用于导入RAG仓库的app", version="v1.0")

    # 2. 跨域配置   allow_credentials改为True则客户端可以带信息过来，但是其他三个就必须得明确指定
    app.add_middleware(
        CORSMiddleware,
...
def register_router(app: FastAPI):
    ...
    def upload_endpoint(file: UploadFile,
        ...
        # 2. 运行整个导入的图谱(耗时：节点多【pdf解析很慢】),所以放到后台任务慢慢做
