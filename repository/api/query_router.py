...
from repository.schema.query_schema import QueryRequest, StreamSubmitResponse, QueryResponse
from repository.core.deps import get_query_service
from repository.service.query_service import QueryService
from repository.utils.sse_util import create_sse_queue, sse_generator
...
def create_app():
    ...
    # 1. 实例化
    app = FastAPI(description="用于查询商品问题的app", version="v1.0")
...
def register_router(app: FastAPI):
    ...
    async def query(request: QueryRequest,
        ...
        # 3.2 非流式调用(直接用当前线程运行查询流程 不启动一个新线程执行很不合理：因为不是只有流式慢 非流式也慢，所以也要创建一个新线程执行)
        else:
